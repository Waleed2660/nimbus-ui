...
let currentWorkingDirectory = "Nimbus/Bin";
...
const formatFileName = (fileName) => {
  return fileName.replace(/^Nimbus\/Bin\//, "");
}
...
function FilesPage(pwd) {
  ...
  const gotoPreviousDirectory = () => {
    if (currentWorkingDirectory === "Nimbus/Bin") {
      return;
    }
        ...
        <table className="min-w-full text-gray-600">
          <thead>
            <tr>
              ...
              <th className="table-header w-1/9">File Size {Sort.getSortIcon(sortConfig,'size')}</th>
              <th className="table-header w-1/9 rounded-tr-lg">Kind {Sort.getSortIcon(sortConfig,'kind')}</th>
            </tr>
          </thead>
...
const displayLoadingAnimation = () => {
  return (
    <div className="flex items-center justify-center min-h-screen w-full ml-80">
      <svg aria-hidden="true" className="w-10 h-10 animate-spin fill-green-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
